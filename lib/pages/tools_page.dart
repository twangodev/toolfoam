...
class _ToolsPageState extends State<ToolsPage> {
  ...
  void onAddTool() async {
    TfTool(id: TfId.unique(), owner: widget.selectedCollection!)
        .create('New Tool');
    await refreshTools();
  }

  void onTap(BuildContext context, TfTool tool) async {
    logger.info('Opening tool ${tool.id} (${tool.metadata.name})');
    await Navigator.push(context,
        MaterialPageRoute(builder: (context) => ToolEditorPage(tool: tool)));
    logger.info('Tool ${tool.id} (${tool.metadata.name}) closed');
    await refreshTools();
  }

  void renameTool(TfTool tool, String name) async {
    logger.fine('Renaming tool ${tool.id} to $name');
    tool.rename(name);
    await tool.push();
  }

  void onDelete(TfTool tool) async {
    logger.fine('Deleting tool ${tool.id} (${tool.metadata.name})');
    tool.delete();
    await refreshTools();
  }
